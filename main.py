...
class AutoDraw(object):
    ...
    def __init__(self, name, blur=0):
        ...
        self.detail = 1 # Madre
        self.scale = 5 / 12
        self.sketch_before = False # Madre
        self.with_color = True
        self.num_colors = 5
        self.outline_again = False # Madre
        ...
        pseudo_x = int(self.img_shape[1] * self.detail)
    ...
    def show(self):
        cv2.imshow('image', np.swapaxes(self.img, 0, 1))
        cv2.waitKey(0)
    ...
    def execute(self, commands):
        ...
        for (i, comm) in enumerate(commands):
            if type(comm) == str:
                ...
                if press == 0:
                    pg.moveTo(comm[0], comm[1], 0.000005)
                else:
                    pg.dragTo(comm[0], comm[1], 0.000005)
        return
    ...
    def run(self):
        if self.with_color:
            ...
            for (i, color) in enumerate(colors):
                ...
                for j in str(round(color[2])):
                    pg.press(j)

                pg.click(x = 1673, y = 1261)
                ...
                self.commands = []
                self.curr_pos = (pg.size()[0]/2, pg.size()[1]/2)
                point = self.translate(self.curr_pos)
                self.commands.append(point)
                self.commands.append("UP")
                self.createPath()

                print('Ready! 5 seconds until drawing begins...')
                time.sleep(5)
